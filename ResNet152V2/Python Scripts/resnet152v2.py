...
def build_resnet152v2(num_classes=1000, input_shape=(224, 224, 3)):
    """
    ResNet-152 V2 — Identity Mappings in Deep Residual Networks.
    Paper: He et al., ECCV 2016.

    Identical to ResNet-101 V2 except:
    ...
    Key V2 differences vs V1:
      ...
      - Post-processing: BN + ReLU added after final stage
      - Identity shortcuts carry pure gradient signal

      Stem    : Conv7x7/2  (no BN/ReLU)  + MaxPool3x3/2  ->   64 x 56x56
      Stage 1 : 3  x PreActBottleneck(64)   stride=1      ->  256 x 56x56
      Stage 2 : 8  x PreActBottleneck(128)  stride=2      ->  512 x 28x28
      Stage 3 : 36 x PreActBottleneck(256)  stride=2      -> 1024 x 14x14
    ...
    for i in range(2, 9):                                           # blocks 2-8
        x = _pre_act_bottleneck(x, 128, stride=1, conv_shortcut=False, name=f'conv3_block{i}')
    ...
    return keras.Model(inputs=inputs, outputs=outputs, name='resnet152v2')
