"""Train EfficientNetV2B2 from scratch."""
import tensorflow as tf
from tensorflow import keras
from tensorflow.keras.preprocessing.image import ImageDataGenerator
from tensorflow.keras.callbacks import ModelCheckpoint, ReduceLROnPlateau
from EfficientNetV2B2 import build_efficientnetv2b2

TRAIN_DIR="dataset/train"; VAL_DIR="dataset/val"; IMG_SIZE=(260,260); BATCH=32; EPOCHS=30
...
model=build_efficientnetv2b2(num_classes=NUM_CLASSES)
model.compile(optimizer=keras.optimizers.Adam(1e-3),loss="categorical_crossentropy",metrics=["accuracy"])
cbs=[ModelCheckpoint("efficientnetv2b2_best.keras",monitor="val_accuracy",save_best_only=True,verbose=1),
     ReduceLROnPlateau(monitor="val_loss",factor=0.1,patience=5,min_lr=1e-7,verbose=1)]
model.fit(train_data,validation_data=val_data,epochs=EPOCHS,callbacks=cbs)
print("Done. Saved: efficientnetv2b2_best.keras")
