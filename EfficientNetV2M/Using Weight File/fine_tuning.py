"""Two-phase fine-tuning for EfficientNetV2M."""
...
# ── Phase 1: head only ────────────────────────────────────────────────────────
base_model = EfficientNetV2M(weights="imagenet", include_top=False, input_shape=(384, 384, 3))
...
model   = keras.Model(inputs, outputs, name="EfficientNetV2M_ft")

model.compile(optimizer=keras.optimizers.Adam(1e-3),
              loss="categorical_crossentropy", metrics=["accuracy"])
cb1 = [ModelCheckpoint("efficientnetv2m_ft_p1.keras", monitor="val_accuracy", save_best_only=True, verbose=1),
...
# ── Phase 2: unfreeze last 40% of backbone (stages 5-6 + head conv) ──────────
n = len(base_model.layers)
unfreeze_from = int(n * 0.60)
...
model.compile(optimizer=keras.optimizers.Adam(1e-5),
              loss="categorical_crossentropy", metrics=["accuracy"])
cb2 = [ModelCheckpoint("efficientnetv2m_ft_best.keras", monitor="val_accuracy", save_best_only=True, verbose=1),
...
print("Done. Best model: efficientnetv2m_ft_best.keras")
