...
def _bottleneck(x, filters, stride=1, name=None):
    """
    ResNet50 Bottleneck block: Conv1x1 -> Conv3x3 -> Conv1x1 + residual.
    Projection shortcut is applied when stride != 1 or channels change.
...
def build_resnet50(num_classes=1000, input_shape=(224, 224, 3)):
    """
    ResNet-50 — Deep Residual Learning for Image Recognition.
    Paper: He et al., CVPR 2016.

    Architecture (Bottleneck blocks, expansion=4):
      ...
      Stage 3 : 6 x Bottleneck(256) stride=2           ->1024 x 14x14
    ...
    # ── Stage 1: 3 blocks, 64 filters, no spatial downsample ──
    ...
    x = _bottleneck(x, 256, stride=1, name='conv4_block2')
    x = _bottleneck(x, 256, stride=1, name='conv4_block3')
    x = _bottleneck(x, 256, stride=1, name='conv4_block4')
    x = _bottleneck(x, 256, stride=1, name='conv4_block5')
    x = _bottleneck(x, 256, stride=1, name='conv4_block6')
    ...
    return keras.Model(inputs=inputs, outputs=outputs, name='resnet50')
