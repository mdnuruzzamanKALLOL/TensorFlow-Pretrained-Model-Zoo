"""ImageNet-pretrained EfficientNetV2B3 — head-only training."""
...
model=keras.Model(inputs,outputs,name="EfficientNetV2B3_pretrained")
model.compile(optimizer=keras.optimizers.Adam(1e-3),loss="categorical_crossentropy",metrics=["accuracy"])
cbs=[ModelCheckpoint("efficientnetv2b3_pretrained.keras",monitor="val_accuracy",save_best_only=True,verbose=1),
     ReduceLROnPlateau(monitor="val_loss",factor=0.1,patience=5,min_lr=1e-7,verbose=1)]
model.fit(train_data,validation_data=val_data,epochs=10,callbacks=cbs)
print("Saved: efficientnetv2b3_pretrained.keras")
