"""Two-phase fine-tuning for EfficientNetV2B3."""
...
TRAIN_DIR="dataset/train"; VAL_DIR="dataset/val"; IMG_SIZE=(300,300); BATCH=16
...
inputs=keras.Input(shape=(300,300,3)); x=base_model(inputs,training=False)
x=layers.GlobalAveragePooling2D()(x); x=layers.Dropout(0.3)(x)
outputs=layers.Dense(NUM_CLASSES,activation="softmax")(x)
model=keras.Model(inputs,outputs,name="EfficientNetV2B3_ft")
model.compile(optimizer=keras.optimizers.Adam(1e-3),loss="categorical_crossentropy",metrics=["accuracy"])
cb1=[ModelCheckpoint("efficientnetv2b3_ft_p1.keras",monitor="val_accuracy",save_best_only=True,verbose=1),
...
cb2=[ModelCheckpoint("efficientnetv2b3_ft_best.keras",monitor="val_accuracy",save_best_only=True,verbose=1),
...
print("Done. Best: efficientnetv2b3_ft_best.keras")
