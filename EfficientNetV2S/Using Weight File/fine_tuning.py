"""Two-phase fine-tuning for EfficientNetV2S."""
...
model   = keras.Model(inputs, outputs, name="EfficientNetV2S_ft")

model.compile(optimizer=keras.optimizers.Adam(1e-3),
              loss="categorical_crossentropy", metrics=["accuracy"])
cb1 = [ModelCheckpoint("efficientnetv2s_ft_p1.keras", monitor="val_accuracy", save_best_only=True, verbose=1),
...
# ── Phase 2: unfreeze last 40% of backbone (stages 4-5 + head conv) ──────────
...
model.compile(optimizer=keras.optimizers.Adam(1e-5),
              loss="categorical_crossentropy", metrics=["accuracy"])
cb2 = [ModelCheckpoint("efficientnetv2s_ft_best.keras", monitor="val_accuracy", save_best_only=True, verbose=1),
...
print("Done. Best model: efficientnetv2s_ft_best.keras")
